...
COMMONVOICE_DEFAULT_SPLITS = ("test", "dev", "small")
...
def _read_cv_manifests_if_cached(
    ...
    for part in ["test", "dev", "small"]:
        for manifest in ["recordings", "supervisions"]:
            path = output_dir / f"{dataset}_{manifest}_{part}.jsonl.gz"
            if not path.is_file():
                continue
...
def _parse_utterance(
    ...
    audio_path = lang_path / audio_info["AUDIO"]
    # print("audio_path: ", audio_path)
    if not audio_path.is_file():
        logging.info(f"No such file: {audio_path}")
        return None

    recording_id = Path(audio_info["AUDIO"]).stem
    ...
    segment = SupervisionSegment(
        ...
        channel=0,
        speaker=audio_info["ID"],
        text=audio_info["TEXT"].strip(),
    )
...
def _prepare_part(
    ...
    tsv_path = lang_path / part / "metadata_cleaned.tsv"
...
def prepare_annaspeech(
    ...
    lang_manifests = _read_cv_manifests_if_cached(output_dir=output_dir)
    lang_path = corpus_dir

    for part in tqdm(splits, desc=f"Spliting"):
        logging.info(f"Spliting {part}")
        if part in lang_manifests:
            logging.info(f"{part} split of CommonVoice- already prepared - skipping.")
            continue
        ...
        supervision_set.to_file(output_dir / f"{dataset}_supervisions_{part}.jsonl.gz")
        recording_set.to_file(output_dir / f"{dataset}_recordings_{part}.jsonl.gz")
